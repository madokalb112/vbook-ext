...
function comicChapterName(a) {
    let no = firstText(a, [".ep-row-v2-no"]);
    let title = firstText(a, [".ep-row-v2-title strong", ".ep-row-v2-title"]);
    let sub = firstText(a, [".ep-row-v2-title span.sub"]);
    let date = firstText(a, [".ep-row-v2-date"]);
    let name = "";
    if (no && title) name = no + " - " + title;
    else name = title || cleanText(a.text());
    if (sub && name.indexOf(sub) < 0) name += " - " + sub;
    if (date) name += " (" + date + ")";
    return cleanText(name);
}

function novelChapterName(a) {
    let no = firstText(a, [".ne-num"]);
    let title = firstText(a, [".ne-title"]);
    let date = firstText(a, [".ne-date"]);
    let name = "";
    if (no && title) name = no + " - " + title;
    else name = title || no || cleanText(a.text());
    if (date) name += " (" + date + ")";
    return cleanText(name);
}

function collectComicToc(doc) {
    let data = [];
    let seen = {};
    let nodes = doc.select("ul.ep-list-v2 a.ep-row-v2-link[href]");
    for (let i = 0; i < nodes.size(); i++) {
        let a = nodes.get(i);
        let href = normalizeUrl(a.attr("href"));
        if (!isComicChapterUrl(href) || seen[href]) continue;
        seen[href] = true;
        data.push({name: comicChapterName(a), url: href, host: BASE_URL});
    }
    return data;
}

function collectNovelToc(doc) {
    let data = [];
    let seen = {};
    let nodes = doc.select("ul.novel-eps li a[href], ul.novel-eps a[href]");
    for (let i = 0; i < nodes.size(); i++) {
        let a = nodes.get(i);
        let href = normalizeUrl(a.attr("href"));
        if (!isNovelChapterUrl(href) || seen[href]) continue;
        seen[href] = true;
        data.push({name: novelChapterName(a), url: href, host: BASE_URL});
    }
    return data;
}

function shouldReverse(data) {
    if (data.length < 2) return false;
    let first = chapterNumber(data[0].name);
    let last = chapterNumber(data[data.length - 1].name);
...
function execute(url) {
    ...
    let doc = null;
    if (isComicStoryUrl(story)) {
        doc = getDoc(story + (story.indexOf("?") >= 0 ? "&" : "?") + "sort=asc");
    }
    if (!doc) doc = getDoc(story);
    if (!doc) return Response.error(docErrorMessage("Khong lay duoc danh sach chapter."));

    let data = isNovelStoryUrl(story) ? collectNovelToc(doc) : collectComicToc(doc);
    if (data.length === 0) {
        data = collectComicToc(doc);
        if (data.length === 0) data = collectNovelToc(doc);
    }
    if (shouldReverse(data)) data.reverse();
