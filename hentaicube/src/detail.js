...
function getGenres(doc) {
    let data = [];
    let seen = {};
    doc.select(".genres-content a[href], .post-content_item a[href*='/theloai/'], a[href*='/theloai/']").forEach(function(a) {
...
function latestChapters(doc) {
    let parts = [];
    let seen = {};
    doc.select(".wp-manga-chapter a[href], .chapter-list a[href], .chapter-item a[href], a[href*='/chap-'], a[href*='/oneshot']").forEach(function(a) {
...
function execute(url) {
    ...
    if (!name) name = metaContent(doc, "meta[property=og:title], meta[name=twitter:title]").replace(/\s+\|\s+HentaiCB$/i, "").replace(/\s+-\s+HentaiCB$/i, "");
    ...
    let author = valueByLabels(doc, ["author", "tac gia", "artist"]);
    if (!author) author = cleanText(doc.select(".author-content a").text());
    let status = valueByLabels(doc, ["status", "tinh trang"]);
    ...
    if (latest) detail += (detail ? "<br>" : "") + "<b>Chap moi:</b><br>" + latest;
    ...
    return Response.success({
        ...
        detail: detail,
        ongoing: statusFold.indexOf("hoan thanh") < 0 && statusFold.indexOf("completed") < 0 && statusFold.indexOf("complete") < 0 && statusFold.indexOf("ended") < 0,
