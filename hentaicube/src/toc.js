...
function parseChapters(doc) {
    let data = [];
    let seen = {};
    doc.select("li.wp-manga-chapter a[href], .wp-manga-chapter a[href], .chapter-list a[href], .listing-chapters_wrap a[href], .chapter-item a[href], a[href*='/chap-'], a[href*='/oneshot']").forEach(function(a) {
...
function execute(url) {
    ...
    let text = fetchAjaxHtml(chaptersUrl(target), target);
    if (!text) {
        try {
