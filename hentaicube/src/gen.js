...
function itemDescription(e) {
    let parts = [];
    e.select(".list-chapter a[href], .chapter a[href], .wp-manga-chapter a[href], .chapter-item a[href], a[href*='/chap-']").forEach(function(a) {
...
function validName(name) {
    ...
    return name && folded.indexOf("chapter") < 0 && folded.indexOf("chap ") < 0 && folded.indexOf("read first") < 0 && folded.indexOf("read last") < 0;
}
...
function execute(url, page) {
    let target = listPageUrl(url || "/read/", page);
