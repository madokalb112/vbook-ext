...
function execute(key, page) {
    ...
    var offset = (p - 1) * 20;
    var urls = [
        API_URL + "/comic/search?q=" + encodeURIComponent(key || "") + "&offset=" + offset + "&limit=20",
        API_URL + "/comic/search?keyword=" + encodeURIComponent(key || "") + "&offset=" + offset + "&limit=20",
        API_URL + "/search?q=" + encodeURIComponent(key || "") + "&offset=" + offset + "&limit=20"
    ];
    var json = null;
    for (var u = 0; u < urls.length && !json; u++) json = fetchJson(urls[u]);
    if (!json) return Response.error("Cannot search Comicland.");
