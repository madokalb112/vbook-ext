...
function isChapterUrl(url) {
    ...
    return /^\/truyen-tranh\/[^/]+\/chuong-[^/]+$/i.test(path);
}
...
function chapterNumber(urlOrText) {
    let match = /chuong[-\s]*(\d+(?:[.-]\d+)?)/i.exec(foldText(urlOrText || ""));
    if (!match) match = /(\d+(?:[.-]\d+)?)/.exec(urlOrText || "");
...
function addPageParam(url, page) {
    url = normalizeUrl(url || OFFICIAL_URL);
...
function listPageUrl(url, page) {
    return addPageParam(url || OFFICIAL_URL, page);
}
...
function imageHeaders(referer) {
    return {"Referer": normalizeUrl(referer || OFFICIAL_URL), "User-Agent": DESKTOP_UA};
}

function execute() {
    return Response.success({baseUrl: BASE_URL, homeUrl: OFFICIAL_URL, ok: true});
}
